...
import type { ParsedStepperModel, StepperModel } from './Stepper.interfaces';
import type { MultipleChoiceValues } from './Stepper.types';
...
export async function sendSMS(phone: string): Promise<Response | undefined> {
    try {
        ...
        if (phone !== '') {
            ...
            const response = await fetch(`${import.meta.env.PUBLIC_API_URL}/TwilioVerifyPhoneNumber`, {
                method: 'POST',
...
export async function verifySMSCode(phone: string, code: string): Promise<Response | undefined> {
    try {
        ...
        if (phone !== '' && code !== '') {
            ...
            const response = await fetch(`${import.meta.env.PUBLIC_API_URL}/TwilioVerifyCode`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
export async function addBitrixContactDeal(data: ParsedStepperModel): Promise<Response | undefined> {
    try {
        ...
        const response = await fetch(`${import.meta.env.PUBLIC_API_URL}/DAABitrix`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
